...
<h3> Page to Upadate Teacher Details </h3>
<form method="POST">
<select name="name">
<?php
...
<?php	echo '<select name="column">
<option value="tname">TeacherName</option>
<option value="exp">Experience</option>
<option value="subjects">Subjects Handling</option>
</select>
<input id="newhr" class="form-control input-lg" name="newdata" type="text" placeholder="Enter NewDetails" >
<button type="submit" name="update"><b>Update Details</b></h4></button>';
 mysqli_free_result($result);
 }
 if (isset($_POST['update'])) {
    $data = new Teacher();
    $result = $data->updateTeacher($_POST['name'], $_POST['column'], $_POST['newdata']);
    if($result){
